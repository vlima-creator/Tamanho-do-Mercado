...
# -*- coding: utf-8 -*-
"""
Módulo de cálculos de mercado e análise estratégica - Suporte a Múltiplas Categorias e Visão Mensal
"""
...
class MarketAnalyzer:
    """Classe para análise de mercado e cálculo de scores com suporte a múltiplas categorias e visão mensal"""
    ...
    def get_consolidado_6m(self, categoria: str, subcategoria: str) -> Dict:
        """Consolida os últimos 6 meses para manter compatibilidade com ranking original"""
        ...
        # Pega os últimos 6 registros (assumindo que são mensais)
        df_6m = df.tail(6)
        faturamento_6m = float(df_6m['faturamento'].sum())
        unidades_6m = int(df_6m['unidades'].sum())
    ...
    def calcular_score(self, categoria: str, faturamento_6m: float, ticket_mercado: float) -> float:
        ...
        if diff_pct <= range_pct:
            u = 1.0
        elif ticket_cliente < float(ticket_mercado):
            u = 0.7 
        else:
            u = 0.3 
    ...
    def gerar_ranking(self, categoria: str = None) -> pd.DataFrame:
        """Gera ranking de subcategorias com base no consolidado de 6 meses"""
    ...
    def simular_cenarios(self, categoria: str, subcategoria: str, custom_shares: Dict = None) -> Dict:
        """Simula cenários de crescimento baseados no consolidado de 6 meses"""
        cons = self.get_consolidado_6m(categoria, subcategoria)
        mercado_6m = float(cons['faturamento_6m'])
        margem = float(self.cliente_data.get('margem', 0.0))
        fat_3m = float(self.cliente_data.get('faturamento_3m', 0.0))
        faturamento_base_comparacao = fat_3m * 2
        ...
        for nome, info in shares.items():
            ...
            delta = receita_proj - faturamento_base_comparacao
            crescimento = (delta / faturamento_base_comparacao * 100) if faturamento_base_comparacao > 0 else 0.0
    ...
    def identificar_anomalias(self, categoria: str) -> List[Dict]:
        """Identifica anomalias no mercado da categoria"""
        anomalias = []
        ...
        for _, row in df_subs.iterrows():
            if float(row['ticket_medio']) > avg_ticket * 1.5:
                anomalias.append({
                    ...
                    'Tipo': 'Ticket Elevado',
                    'Impacto': 'Alto',
                    'Insight': 'Ticket muito acima da média da categoria.'
                })
        return anomalias

    def editar_mercado_categoria(self, categoria: str, novos_dados: List[Dict]):
        """Permite edição manual dos dados de mercado da categoria"""
        self.mercado_categoria[str(categoria)] = novos_dados
